...
const Vesting = artifacts.require('Payroll');
...
contract('Payroll contract tests', function ([admin, admin2, dao, beneficiary, random, ...otherAccounts]) {
  ...
  beforeEach(async () => {
    ...
    this.vesting = await PayrollWithFixedTime.new(
  ...
  it('Can create a worker schedule based on experience level', async () => {
    // this will create schedule #0
    await this.vesting.createPayroll(
      this.mockToken.address,
      beneficiary,
      '5',
      '100',
    ...
    const {
      ...
      _amount,
      _drawDownRate
    } = await this.vesting.vestingSchedule('0');
  ...
  it('Can create a worker schedule and DXD schedule based on experience level', async () => {
    ...
    await this.vesting.createPayrollAndDxd(
      this.mockToken.address,
      beneficiary,
      '5',
      '100',
    ...
    const scheduleErc20 = await this.vesting.vestingSchedule('0');
    ...
    const scheduleDxd = await this.vesting.vestingSchedule('1');
  ...
  it('Can update the salary of a worker ', async () => {
    ...
    const existingSalary = await this.vesting.workerExperienceLevelToSalary(levelBeingUpdated);
    expect(existingSalary).to.be.bignumber.equal(to18dp(experienceToSalary[levelBeingUpdated]));

    const newSalary = to18dp('500');
    await this.vesting.updateWorkerExperienceLevelSalary(levelBeingUpdated, newSalary);

    const updatedSalary = await this.vesting.workerExperienceLevelToSalary(levelBeingUpdated);
    expect(updatedSalary).to.be.bignumber.equal(newSalary);
  });

  it('Reverts when updating salary as non admin', async () => {
    await expectRevert(
      this.vesting.updateWorkerExperienceLevelSalary('5', '5', {from: beneficiary}),
